...
**和 Function Calling 的区别及选型**：

| 维度 | Function Calling（本地工具） | MCP（远端工具） |
|------|--------------------------|----------------|
| 工具定义 | 本地代码 @Tool 注解，自己实现 | 远端 MCP Server 提供，动态下发 |
| 适用场景 | 需要访问项目内部数据（数据库、业务逻辑） | 需要调用外部能力（联网搜索、第三方服务） |
| 维护成本 | 自己维护，灵活可控 | 依赖第三方 Server，无需自己实现 |
| 项目用途 | 餐厅预订、个性化推荐（查自己的 DB） | 联网搜索（查实时网络信息） |

项目里两种方式都用了，各司其职：餐厅预订和推荐需要查项目自己的数据库，用本地 @Tool；联网搜索需要访问外部网络，用 MCP 接入智谱 BigModel，不需要自己实现搜索逻辑。
...
## Q：为什么选通义千问而不是 GPT 或其他模型？

| 维度 | 通义千问（DashScope） | GPT-4 / Claude |
|------|---------------------|----------------|
| 中文能力 | 强，专门针对中文优化 | 较强，但中文不如英文稳定 |
| 访问方式 | 国内直连，无需代理 | 国内需要代理，不稳定 |
| 价格 | 相对低，有免费额度 | 较贵 |
| Function Calling | 支持，中文 Prompt 下工具调用稳定 | 支持，英文 Prompt 下更稳定 |
| 生态 | LangChain4j 有官方 DashScope Starter | LangChain4j 也支持 OpenAI |

项目选通义千问的核心原因：国内开发环境直连稳定，不需要代理；中文场景下工具调用理解更准确；有免费额度适合开发测试。@Tool 描述里用英文写，是因为通义千问对英文工具描述的理解比中文更稳定，这是实测结论。
